...
def detect_tubes_by_contour(image, gray, threshold_val, min_area, max_area, 
                           min_solidity, max_aspect_ratio, max_circularity):
    """
    Detect tubes using contour analysis instead of circles.
    Tubes are white/bright octagonal shapes, not perfect circles.
    """
    ...
    for contour in contours:
        ...
        # Tubes should have low circularity (not perfectly round)
        ...
        # Get center and approximate radius for display
...
def main():
    ...
    window_name = "EP Tube Detection (Contour-Based)"
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    
    # Create trackbars for detection parameters
    cv2.createTrackbar("Threshold", window_name, 150, 255, nothing)
    cv2.createTrackbar("Min Area", window_name, 500, 5000, nothing)
    cv2.createTrackbar("Max Area", window_name, 3000, 10000, nothing)
    cv2.createTrackbar("Min Solidity", window_name, 60, 100, nothing)  # 0-100 as percent
    cv2.createTrackbar("Max Aspect", window_name, 30, 50, nothing)  # 1.0-5.0
    
    print("=" * 70)
    print("EP TUBE DETECTION - Contour-Based (Octagonal Shape)")
    print("=" * 70)
    print("\nTrackbars created. Adjust parameters in real-time.")
    print("Press 'q' to quit and print final parameters.")
    print("\nParameter ranges:")
    print("  - Threshold: 0-255 (brightness cutoff for white tubes)")
    print("  - Min Area: 0-5000 pixels")
    print("  - Max Area: 0-10000 pixels")
    print("  - Min Solidity: 0-100% (how filled the shape is)")
    print("  - Max Aspect: 1.0-5.0 (width/height ratio)")
    print("-" * 70)
    ...
    while True:
        ...
        # Detect tubes
        tubes, binary = detect_tubes_by_contour(
            image, gray, threshold, min_area, max_area,
            min_solidity, max_aspect, max_circularity=0.5
        )
        ...
        if tubes:
            for tube in tubes:
                ...
                cv2.circle(display_image, tube['center'], 3, (0, 0, 255), -1)
        ...
        info_text = (f"Threshold:{threshold} MinArea:{min_area} MaxArea:{max_area} "
                    f"MinSol:{min_solidity:.2f} MaxAsp:{max_aspect:.1f}")
        ...
        if key == ord('q'):
            ...
            print(f"Min Solidity:     {min_solidity:.2f} (0.0-1.0)")
            print(f"Max Aspect Ratio: {max_aspect:.2f}")
            print(f"Max Circularity:  0.5 (fixed for octagonal shapes)")
